...
def training_loop(
    ...
    if selection:
        ...
        lr_kwargs.mini_batch_size = mini_batch_size
    ...
    net_beats_ref = False
    lr_kwargs.early = selection_early
    lr_kwargs.late = selection_late
    ...
    state = dnnlib.EasyDict(cur_nimg=0, total_elapsed_time=0, cur_epoch=0, 
                            run_selection=run_selection, is_selection_waiting=is_selection_waiting,
                            change_nimg=0, change_epoch=0,
                            total_time=0, training_time=0, selection_time=0)
    ...
    prev_status_epoch = state.cur_epoch
    prev_status_training_time = state.training_time
    prev_status_time = state.total_time
    start_nimg = state.cur_nimg
    lr = dnnlib.util.call_func_by_name(cur_nimg=state.cur_nimg, cur_epoch=state.cur_epoch, 
                                        change_nimg=state.change_nimg, change_epoch=state.change_epoch,
                                        run_selection=state.run_selection, **lr_kwargs)
    status_logs = {"Stats' Epoch":state.cur_epoch, 'Speed [sec/epoch]':None, 'Speed [sec/kimg]':None, 'Speed [sec/epoch]':None}
    epoch_logs = {"Epoch":state.cur_epoch, "Loss":None, "Seen images [kimg]": 0,
                  "Learning rate":lr, "Training Time [sec]":state.training_time*1000}
    ...
    if selection:
        epoch_logs.update({"Epoch Super-Batch Learner Loss": None, "Epoch Super-Batch Reference Loss": None,
                           "Selection time [sec]":state.selection_time*1000})
    ...
    while True:
        ...
        status_logs.update({"Stats' Epoch": state.cur_epoch})
        ...
        for round_idx in range(num_accumulation_rounds):
            ...
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                ...
                if (state.run_selection and requires_ref_loss) or state.is_selection_waiting:
                    ...
                    if not net_beats_ref and loss.mean() < ref_loss.mean():
                        ...
                        if state.is_selection_waiting:
                            state.run_selection = not(state.run_selection)
                            if state.run_selection: print("Selection will now be run")
                            else: print("Selection will now be stopped")
                            state.is_selection_waiting = False
                            change_just_happened = True
                    # Inform all other GPUs of changes in run_selection
                    if state.is_selection_waiting or change_just_happened:
                        sync_tensor = torch.tensor([state.run_selection], dtype=torch.bool, device=device)
                        torch.distributed.all_reduce(sync_tensor, op=broadcast_operation)
                        new_run_selection = bool(sync_tensor.item())
                        if state.is_selection_waiting and new_run_selection != state.run_selection:
                            net_beats_ref = True
                            print("Network has beaten the reference")
                            ...
                            state.is_selection_waiting = False
                            change_just_happened = True
                        state.run_selection = new_run_selection
                    state.selection_time += (time.time() - selection_time_start)
                elif state.run_selection: ref_loss = None

                # Calculate overall loss
                if state.run_selection:
                    selection_time_start = time.time()
                    try:
                ...
                if change_just_happened:
                    state.change_epoch = state.cur_epoch + round_idx / (num_accumulation_rounds-1)
                    state.change_nimg = state.cur_nimg
                    change_just_happened = False
        ...
        epoch_logs.update({"Training Time [sec]": state.training_time*1000})
        run.log(epoch_logs)
        if status_period is not None and (done or state.cur_epoch % status_period == 0) and (state.cur_nimg != start_nimg or start_nimg == 0):
            ...
            if status_condition:
                ...
                status_logs.update({
                    ...
                    'Speed [sec/epoch]' : (state.training_time - prev_status_training_time) / max(state.cur_epoch - prev_status_epoch, 1),
                    'Speed [sec/kimg]' : (state.training_time - prev_status_training_time) / max(state.cur_nimg - prev_status_nimg, 1) * 1e3,
                })
                run.log(status_logs)
                prev_status_training_time = state.training_time
                prev_status_epoch = state.cur_epoch
