...
def do_train(
    ...
    acid_rolling=False, acid_iter=512, acid_delay=128, acid_min=512-128, 
    ...
    if acid:
        if acid_late or acid_rolling:
            run_acid = False
            is_acid_waiting = True
            if acid_late:
                log.info("ACID with delayed execution strategy")
            else:
                log.info("Rolling ACID with delayed execution strategy")
        elif acid_early:
            run_acid = True
            is_acid_waiting = True
    ...
    if guidance and acid:
        ...
        ref = None
        if acid_rolling: 
            log.warning("Rolling ACID reference will be assigned during training")
            log.info("Rolling ACID iteration period = %s", acid_iter)
            log.info("Rolling ACID min iteration = %s", acid_min)
            log.info("Rolling ACID iteration delay = %s", acid_delay)
    ...
    for iter_idx in progress_bar:
        ...
        if run_acid or is_acid_waiting:
            if acid_rolling:                
                ...
                if iter_idx >= acid_min and (iter_idx+1) % acid_iter == 0:
                    ref = copy.deepcopy(net).eval().requires_grad_(False)
                    log.warning("Updated ACID Bad Version model")
                if is_acid_waiting and iter_idx >= acid_min + acid_delay:
                    is_acid_waiting = False
                    run_acid = True
            if ref is not None:
                ...
                if not net_beats_ref and net_loss.mean() < ref_loss.mean():
                    ...
                    if is_acid_waiting and not acid_rolling:
